...
class Trainer:
    ...
    def create_optimizer(self):
        """
        Setup the optimizer.
        修改说明：将参数分为骨干网络和分类头两部分，分别赋予不同的学习率。
        """
        ...
        if self.optimizer is None:
            ...
            # 默认分类头学习率是基础的10倍，或者从 args.head_lr 中获取
            head_lr = getattr(self.args, "head_lr", base_lr * 4)
            head_keywords = ["classifier", "pooler"]

            optimizer_grouped_parameters = [
                # 1. 骨干网络 / LoRA 适配器 (有 weight decay)
                {
                    "params": [
                        p for n, p in opt_model.named_parameters()
                        if (n in decay_parameters and p.requires_grad and not any(k in n for k in head_keywords))
                    ],
                    "weight_decay": self.args.weight_decay,
                    "lr": base_lr,
                },
                # 2. 骨干网络 / LoRA 适配器 (无 weight decay)
                {
                    "params": [
                        p for n, p in opt_model.named_parameters()
                        if (n not in decay_parameters and p.requires_grad and not any(k in n for k in head_keywords))
                    ],
                    "weight_decay": 0.0,
                    "lr": base_lr,
                },
